...
class to_wrapWithOptionalSkipConnection(nn.Module):
...
def createEdgeUnetBlock(initial_ch, in_ch, out_ch, module_to_wrap):
    ...
    return to_wrapWithOptionalSkipConnection(left, module_to_wrap, right, concat=False)
...
def createIntermediateUnetBlock(in_ch, out_ch, module_to_wrap):
    ...
    return to_wrapWithOptionalSkipConnection(left, module_to_wrap, right, concat=True)
...
def createCentralUnetBlock(in_ch, out_ch):
    ...
    return to_wrapWithOptionalSkipConnection(left, None, right, concat=True)
...
class Predictor(nn.Module):
    def __init__(self, in_ch=6, feature_size=64, out_ch=3):
        ...
        self.model = createEdgeUnetBlock(6, feature_size, 3, to_wrap)
